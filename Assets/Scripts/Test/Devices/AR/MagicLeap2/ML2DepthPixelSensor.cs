...
public class ML2DepthPixelSensor : MonoBehaviour
{
    ...
    private Vector2[] _undistortionLookupTable;
    private Vector2 _focalLength;
    ...
    {
        if (_undistortionLookupTable == null || _lastFrameData == null) return null;
        return DepthUndistortion.UndistortDepthImageFast(_lastFrameData, _width, _height, _undistortionLookupTable);
    }
        ...
        while (pixelSensorFeature.GetSensorStatus(sensorId.Value) == PixelSensorStatus.Started)
        {
            foreach (uint stream in configuredStreams)
            {
                        ...
                        Allocator.Temp, shouldFlipTexture: true))
                {
                    ...
                    // Debug.Log($"Depth Pixel Sensor Pose: Position {sensorPose.position} Rotation: {sensorPose.rotation}");
                ...
                _width = (int)frame.Planes[0].Width;
                _height = (int)frame.Planes[0].Height;

                _lastFrameData = new byte[frame.Planes[0].ByteData.Length];
                frame.Planes[0].ByteData.CopyTo(_lastFrameData);

                if (metaData != null && !setIntrinsics)
                {
                    Debug.Log("[Depth Camera] Looking for Intrinsics...");
                    for (int i = 0; i < metaData.Length; i++)
                    {
                        if (metaData[i] is PixelSensorPinholeIntrinsics)
                        {
                            PixelSensorPinholeIntrinsics intrinsics = (PixelSensorPinholeIntrinsics)metaData[i];
                            _focalLength = intrinsics.FocalLength;

                            DepthUndistortion.CreateUndistortionLookupTable(_width, _height, intrinsics, out _undistortionLookupTable);
                            setIntrinsics = true;
                        }
                    }
                }
    ...
    // =============
    public float GetDepthAtPixel(in byte[] frameData, float xN, float yN)
    {
        int x = (int)(xN * _width);
        int y = (int)(yN * _height);
        
        // Validate coordinates
        if (x < 0 || x >= _width || y < 0 || y >= _height) {
            Debug.LogError($"Pixel coordinates ({x}, {y}) out of bounds. Frame size: {_width}x{_height}");
            return -1f;
        }

        // Convert ByteData to float array with zero allocations
        ReadOnlySpan<byte> byteSpan = frameData.AsSpan();
